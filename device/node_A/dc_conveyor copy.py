...
class DCConveyor:
    ...
    def drop_one(
        ...
        *,
        poll_ms=1,
        hold_ms=30,               # 디바운스 시간
        wait_on_timeout_ms=5000,  # S7 ON 확정 대기
        drop_timeout_ms=3000,     # S7 ON 이후 drop 완료 제한
        tail_run_ms=2000,
    ):
        ...
        while True:
            ...
            if tail_deadline is None:
                if filt.falling(raw):
                    print("[CONV] S7 stable OFF -> tail run {}ms".format(tail_run_ms))
                    tail_deadline = time.ticks_add(now, tail_run_ms)

            else:
                if time.ticks_diff(now, tail_deadline) >= 0:
                    self.stop()
                    print("[CONV] drop complete (tail done)")
                    return True

            if time.ticks_diff(now, t_drop0) > drop_timeout_ms:
                print("[CONV][ERR] drop timeout -> STOP")
                self.stop()
                return False
    ...
    while True:
        ...
        if cmd == "d":
            ...
            conv.drop_one(di,
                ...
                drop_timeout_ms=6000,      # S7 ON 이후, S7 OFF + tail 완료 최대 시간
                tail_run_ms=3000,          # S7 OFF 이후 추가 구동 시간
            )
