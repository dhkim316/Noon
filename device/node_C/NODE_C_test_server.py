...
def input_loop(conn, disconnected):
    print("commands: <flavor> <side>  (example: f1 left), q=quit")
    while True:
        ...
        parts = cmd.split()
        if len(parts) != 2:
            print("use: f1 left | f1 right | f2 left | f2 right | f3 left | f3 right | f4 left | f4 right")
            continue
        ...
        op = "bottle.fill.{}.{}".format(flavor, side)
        try:
            send_msg(conn, make_msg("req", op, {}))
        except OSError as exc:
